...
function Component() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = new FormData(e.currentTarget) // if we use "e.currentTarget" > it works
    // const formData = new FormData(e.target) // but if we use "e.target" > it won't work
    // const formData = new FormData(e.target as HTMLFormElement) // need to use "type assertion" if we use "e.target"

    const data = Object.fromEntries(formData)
    console.log(data)

    // const text = formData.get('text') // it shows err
    const text = formData.get('text') as string // this is a way to walk around the err above > we know that this is string
